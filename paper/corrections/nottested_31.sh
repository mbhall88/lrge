#!/usr/bin/env bash
# Issue #29 "Not tested" section: do the 7 remaining PacBio two-set failures recover under -F?
# Prep mirrors paper/workflow/scripts/download.sh; estimates use the merged-main binary,
# which contains the #31 fix (fc09157 + 7af167a).
set -euxo pipefail
...
num_seqs=$(awk 'NR==2 {print $4}' "${run}.stats.tsv")
echo "[INPUT] $run num_seqs=$num_seqs num_bases=$num_bases"
...
variant () {
  ...
  echo "[RESULT] $run $tag estimate=$(cat "${run}.${tag}.size") n_est=$(wc -l < "${run}.${tag}.perread.txt") n_inf=$(grep -c inf "${run}.${tag}.perread.txt" || true)"
}
