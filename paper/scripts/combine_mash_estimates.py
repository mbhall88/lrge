...
def main():
    ...
    print("sample,sketch_size,min_copies,estimate,memory,cpu_time")
    for log in indir.rglob("*.log"):
        sample = log.parts[-2]
        min_copies = log.parts[-3][1:]
        sketch_size = log.parts[-4][1:]
        est = None
        memory = None
        sys_time = None
        user_time = None
        with open(log) as f:
            ...
            for line in lines:
                if "Estimated genome size:" in line:
                    est = int(float(line.strip().split()[-1]))
        ...
        print(
            ",".join(
                [sample, sketch_size, min_copies, str(est), str(memory), str(cpu_time)]
            )
        )
